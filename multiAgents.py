    ...
    def getAction(self, gameState):
        ...
        def maxValue(gameState, depth, numGhosts):
            ...
            for action in actions:
                val = max(val, minValue(gameState.generateSuccessor(0, action), depth, 1, numGhosts))
            return val

        def minValue(gameState, depth, agent, numGhosts):
            if gameState.isWin() or gameState.isLose() or depth == 0:
                return self.evaluationFunction(gameState)
            val = float('inf')
            ...
            if agent == numGhosts:
                for action in actions:
                    val = min(val, maxValue(gameState.generateSuccessor(agent, action), depth - 1, numGhosts))
            else:
                for action in actions:
                    val = min(val, minValue(gameState.generateSuccessor(agent, action), depth, agent + 1, numGhosts))
            return val
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        def minValue(gameState, alpha, beta, agent, depth, numGhosts):
            if gameState.isWin() or gameState.isLose() or depth == 0:
                return self.evaluationFunction(gameState)
            val = float('inf')
            actions = gameState.getLegalActions(agent)
            for action in actions:
                ...
                if agent == numGhosts:
                    val = min(val, maxValue(nextState, alpha, beta, depth - 1, numGhosts))
                else:
                    val = min(val, minValue(nextState, alpha, beta, agent + 1, depth, numGhosts))
                if val < alpha:
                    return val
                beta = min(beta, val)
            return val
        ...
        def maxValue(gameState, alpha, beta, depth, numGhosts):
            ...
            actions = gameState.getLegalActions(0)
            for action in actions:
                val = max(val, minValue(gameState.generateSuccessor(0, action), alpha, beta, 1, depth, numGhosts))
                if val > beta:
                    return val
                alpha = max(alpha, val)
            return val
...
def betterEvaluationFunction(currentGameState):
    """
      ...
      DESCRIPTION: We extracted whatever useful information we could think of, then
      set about making a score to score the positive impact of each possible move.
      If he moved closer to the closest pellet, the score increased, while if he
      moved away, the score decreased. Then, in regards to the ghosts, we had the
      score increase if he moved away and decrease if he moved closer in order to
      keep him away from harm. Initially, this only applied if he was 3 spaces away,
      in order to keep him out of immediate danger but let him chase pellets even in
      somewhat risky situations. However, we found that making him more cautious (5
      spaces away) made him score better so here we are.
